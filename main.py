...
class Main:
    ...
    def mage_hero(self):
        ...
        while choose:
            ...
            meciumText = pygame.font.Font('Pixeled.ttf', 15)
    ...
    def barb_hero(self):
        ...
        while choose:
            ...
            for field, value in barb.info_dict.items():
                ...
                makestring = ': '.join(makelist)
                textSurf, textRect = text_objects(makestring, smallText, red)
                textRect.center = (375, (60 + space_counter))
                gameDisplay.blit(textSurf, textRect)
            bio = wrapline(barb.bio,font,500)
            textSurf, textRect = text_objects('BIO:', smallText, red)
            textRect.center = (260, 150)
            gameDisplay.blit(textSurf, textRect)
            for i in bio:
                space_counter += 20
                textSurf, textRect = text_objects(i, smallText, red)
        ...
        while choose:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    quit()
            gameDisplay.fill(black)
            smallText = pygame.font.Font('Pixeled.ttf', 60)
        ...
        while intor:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    quit()
            gameDisplay.fill(black)
            largeText = pygame.font.Font('Pixeled.ttf', 30)
